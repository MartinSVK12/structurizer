...
func _ready():
	...
	var c := Control.new()
	add_child(c)
	_font = c.get_theme_font("font")#_font = load("res://Fonts/font.tres")
	c.queue_free()
...
func set_text(key: String, value, text_color=TEXT_COLOR, bg_color=TEXT_BG_COLOR):
	_texts[key] = {
...
func _on_CanvasItem_draw():
	...
	for key in _texts.keys():
		...
		ci.draw_rect(Rect2(pos, Vector2(ss.x + xpad * 2, line_height)), t.bg_color)
		ci.draw_string(_font, pos + font_offset, text, 0,-1,16, t.text_color)
		pos.y += line_height
